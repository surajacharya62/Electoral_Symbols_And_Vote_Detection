...
class ValidateVote():
    ...
    def validate_single_ballot(self,img, image_name, pred_labels, label_to_id, t_annotation_path):   
        """
        validating the vote whether it is valid or invalid comparing the distance between stamp bounding box with each symbols boundin box
        """
        # args =  parse_args()
        ...
        boxes = pred_labels[0]['boxes'] 
        labels = pred_labels[0]['labels']
        scores = pred_labels[0]['scores']
        # image_name1 = prediction[2]
        # print(image_name, image_name1)            

        df = pd.read_csv(t_annotation_path)
        filtered_rows = df[df['image_id'] == image_name]
        # print(filtered_rows)
        ...
        t_boxes = filtered_rows[['x1', 'y1', 'x2', 'y2']].values.astype('float32')
        
        t_labels = filtered_rows['label'].apply(lambda x: label_to_id[x]).values
        # boxes_as_t = torch.as_tensor(boxes, dtype=torch.float32)
        # labels_as_t = labels.astype(np.int64)
        # labels_as_t = labels.astype(np.int64)
        # labels = torch.as_tensor(labels_as_t, dtype=torch.int64)
        actual_labels = t_labels
        actual_bboxes = t_boxes
        true_labels = zip(t_labels,t_boxes)            

        indices = torch_nms(boxes, scores)   
        final_boxes, final_scores, final_labels  =  boxes[indices], scores[indices], labels[indices] 
        # final_boxes, final_scores, final_labels = viualize.select_highest_confidence_per_class( 
        #                                                 boxes, scores, labels, indices )  
                    
        actual_label = 0
        
        if valid_stamp_id in t_labels:
            actual_label = 'valid'
            actual_label_id = 40
        else:
            actual_label = 'invalid'   
            ...
            if label_id == valid_stamp_id:    
                ...
                if is_stamp_valid(bounding_box, grid_cells):
                    ...
                    if is_valid_symbol:
                        ...
                        # t_box = []
                        # Printing the bounding boxes
                        for bbox in symbol_bounding_boxes:
                            t_box = bbox
                            break
                        t_box = [int(coordinate) for coordinate in t_box]
                        print(t_box)
                        ...
                        print(stamp_box)
                        bounding_boxes_stamp = [bbox for _, bbox in stamp_box]
                        print(bounding_boxes_stamp)
                    

                        if calculate_iou(symbol_box, t_box) > 0.5 and label_id == actual_label_id:
                            print(image_name, actual_labels, label_id, actual_label_id, '----------------------------------' )
